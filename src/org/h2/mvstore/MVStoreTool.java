...
public class MVStoreTool {
    ...
    public static void dump(String fileName, Writer writer, boolean details) {
        ...
        try {
            ...
            for (long pos = 0; pos < fileSize;) {
                ...
                MVChunk c = null;
                try {
                    c = MVChunk.readChunkHeader(block, pos);
                ...
                while (remaining > 0) {
                    ...
                    if (mapId == 0 && details) {
                        if (!compressed) {
                            for (int i = 0; i < entries; i++) {
                                String k = MVStringDataType.INSTANCE.read(chunk);
                                keys[i] = k;
                            }
                        }
                        if (node) {
                            ...
                            for (int i = 0; i < entries; i++) {
                                String v = MVStringDataType.INSTANCE.read(chunk);
                                values[i] = v;
                            }
                            for (int i = 0; i < entries; i++) {
                                pw.println("    " + keys[i] +
                ...
                int footerPos = chunk.limit() - MVChunk.FOOTER_LENGTH;
                try {
                    ...
                    pw.printf(
                            ...
                            new String(chunk.array(), chunk.position(),
                                    MVChunk.FOOTER_LENGTH, DataUtils.LATIN).trim());
                } catch (IllegalArgumentException e) {
                    // too far
    ...
    public static void info(String fileName, Writer writer) {
        ...
        try {
            ...
            TreeMap<Integer, MVChunk> chunks = new TreeMap<Integer, MVChunk>();
            long chunkLength = 0;
            long maxLength = 0;
            ...
            for (Entry<String, String> e : meta.entrySet()) {
                ...
                if (k.startsWith("chunk.")) {
                    MVChunk c = MVChunk.fromString(e.getValue());
                    chunks.put(c.id, c);
            ...
            for (Entry<Integer, MVChunk> e : chunks.entrySet()) {
                MVChunk c = e.getValue();
     ...
     * a byte array.
     */
    static class GenericDataType implements MVDataType {
