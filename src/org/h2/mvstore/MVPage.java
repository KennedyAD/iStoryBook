...
public class MVPage {
     ...
     * @return the new page
     */
    static MVPage createEmpty(MVMap<?, ?> map, long version) {
     ...
     * @return the page
     */
    public static MVPage create(MVMap<?, ?> map, long version,
            Object[] keys, Object[] values, PageReference[] children,
            long totalCount, int memory) {
        MVPage p = new MVPage(map, version);
        // the position is 0
        p.keys = keys;
        p.values = values;
        p.children = children;
     ...
     * @return the page
     */
    public static MVPage create(MVMap<?, ?> map, long version, MVPage source) {
        MVPage p = new MVPage(map, version);
        // the position is 0
        p.keys = source.keys;
     ...
     * @return the page
     */
    static MVPage read(MVFileStore fileStore, long pos, MVMap<?, ?> map,
     ...
     */
    public MVPage getChildPage(int index) {
     ...
     */
    public MVPage copy(long version) {
        MVPage newPage = create(map, version,
                keys, values,
                children, totalCount,
                getMemory());
    ...
    private MVPage splitLeaf(int at) {
    ...
    private MVPage splitNode(int at) {
        int a = at, b = keys.length - a;
     ...
     */
    public void setChild(int index, MVPage c) {
        if (c == null) {
    ...
    public void setKey(int index, Object key) {
        ...
        MVDataType keyType = map.getKeyType();
    ...
    public Object setValue(int index, Object value) {
        ...
        values = values.clone();
        MVDataType valueType = map.getValueType();
     ...
     */
    public void insertNode(int index, Object key, MVPage childPage) {
     ...
     */
    private int write(MVChunk chunk, MVWriteBuffer buff) {
        int start = buff.position();
        int len = keys.length;
    ...
    private void writeChildren(MVWriteBuffer buff) {
        int len = keys.length;
        for (int i = 0; i <= len; i++) {
     ...
     * @param buff the target buffer
     */
    void writeUnsavedRecursive(MVChunk chunk, MVWriteBuffer buff) {
        if (pos != 0) {
    ...
    public boolean equals(Object other) {
        ...
        if (other instanceof MVPage) {
            if (pos != 0 && ((MVPage) other).pos == pos) {
                return true;
            }
            return this == other;
        }
    ...
    public static class PageReference {
        ...
        public PageReference(MVPage page, long pos, long count) {
            this.page = page;
            this.pos = pos;
            this.count = count;
        }
    ...
    public static class PageChildren {
         ...
         */
        static PageChildren read(MVFileStore fileStore, long pos, int mapId,
                long filePos, long maxPos) {
            ByteBuffer buff;
            int maxLength = DataUtils.getPageMaxLength(pos);
            if (maxLength == DataUtils.PAGE_LARGE) {
