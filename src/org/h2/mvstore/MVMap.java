...
public class MVMap<K, V> extends AbstractMap<K, V>
     ...
     */
    protected volatile MVPage root;
    ...
    private final MVDataType keyType;
    private final MVDataType valueType;

    private MVConcurrentArrayList<MVPage> oldRoots =
            new MVConcurrentArrayList<MVPage>();

    private boolean closed;
    private boolean readOnly;
    private boolean isVolatile;

    protected MVMap(MVDataType keyType, MVDataType valueType) {
        this.keyType = keyType;
        this.valueType = valueType;
        this.root = MVPage.createEmpty(this,  -1);
    }
     ...
     */
    synchronized MVPage putBranch(MVPage root, K key, V value) {
        DataUtils.checkArgument(value != null, "The value may not be null");
        long v = writeVersion;
        MVPage p = root.copy(v);
        p = splitRootIfNeeded(p, v);
     ...
     */
    protected MVPage splitRootIfNeeded(MVPage p, long writeVersion) {
        ...
        p = MVPage.create(this, writeVersion,
                keys, null,
                children,
                totalCount, 0);
        return p;
     ...
     */
    protected Object put(MVPage p, long writeVersion, Object key, Object value) {
    ...
    @SuppressWarnings("unchecked")
    private K getMinMax(MVPage p, K key, boolean min, boolean excluding) {
        if (p.isLeaf()) {
            int x = p.binarySearch(key);
            if (x < 0) {
                x = -x - (min ? 2 : 1);
     ...
     */
    protected Object binarySearch(MVPage p, Object key) {
        int x = p.binarySearch(key);
        if (!p.isLeaf()) {
            if (x < 0) {
                x = -x - 1;
     ...
     */
    protected MVPage binarySearchPage(MVPage p, Object key) {
        int x = p.binarySearch(key);
        if (!p.isLeaf()) {
            if (x < 0) {
                x = -x - 1;
    ...
    public synchronized void clear() {
        ...
        newRoot(MVPage.createEmpty(this, writeVersion));
    }
    ...
    public V remove(Object key) {
        ...
        synchronized (this) {
            ...
            if (!p.isLeaf() && p.getTotalCount() == 0) {
                p.removePage();
                p = MVPage.createEmpty(this,  p.getVersion());
            }
            newRoot(p);
        }
        return result;
     ...
     */
    protected Object remove(MVPage p, long writeVersion, Object key) {
        int index = p.binarySearch(key);
        Object result = null;
        if (p.isLeaf()) {
            if (index >= 0) {
        ...
        MVPage cOld = p.getChildPage(index);
     ...
     */
    protected void newRoot(MVPage newRoot) {
        if (root != newRoot) {
            removeUnusedOldVersions();
            if (root.getVersion() != newRoot.getVersion()) {
                MVPage last = oldRoots.peekLast();
     ...
     */
    public MVDataType getValueType() {
        return valueType;
    }
    ...
    void setRootPos(long rootPos, long version) {
        root = rootPos == 0 ? MVPage.createEmpty(this, -1) : readPage(rootPos);
    ...
    public Iterator<K> keyIterator(K from) {
        return new MVCursor<K, V>(this, root, from);
    }
    ...
    private int rewrite(MVPage p, Set<Integer> set) {
        if (p.isLeaf()) {
            long pos = p.getPos();
     ...
     * @return the cursor
     */
    public MVCursor<K, V> cursor(K from) {
        return new MVCursor<K, V>(this, root, from);
    }
    ...
    public Set<Map.Entry<K, V>> entrySet() {
        ...
        final MVPage root = this.root;
        return new AbstractSet<Entry<K, V>>() {

            @Override
            public Iterator<Entry<K, V>> iterator() {
                final MVCursor<K, V> cursor = new MVCursor<K, V>(map, root, null);
    ...
    public Set<K> keySet() {
        ...
        return new AbstractSet<K>() {
            ...
            public Iterator<K> iterator() {
                return new MVCursor<K, V>(map, root, null);
            }
    ...
    private MVPage copy(MVPage source, MVCursorPos parent) {
        MVPage target = MVPage.create(this, writeVersion, source);
        if (source.isLeaf()) {
            MVPage child = target;
            for (MVCursorPos p = parent; p != null; p = p.parent) {
            ...
            MVCursorPos pos = new MVCursorPos(target, 0, parent);
            for (int i = 0; i < getChildPageCount(target); i++) {
                pos.index = i;
