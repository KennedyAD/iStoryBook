...
import org.h2.mvstore.MVPage.PageChildren;
import org.h2.mvstore.cache.MVCacheLongKeyLIRS;
import org.h2.mvstore.type.MVStringDataType;
...
public class MVStore {
     ...
     * number of entries.
     */
    private MVCacheLongKeyLIRS<MVPage> cache;
     ...
     */
    private MVCacheLongKeyLIRS<PageChildren> cacheChunkRef;

    /**
     * The newest chunk. If nothing was stored yet, this field is not set.
     */
    private MVChunk lastChunk;

    /**
     * The map of chunks.
     */
    private final ConcurrentHashMap<Integer, MVChunk> chunks =
            new ConcurrentHashMap<Integer, MVChunk>();
    ...
    private final ConcurrentHashMap<Long,
            HashMap<Integer, MVChunk>> freedPageSpace =
            new ConcurrentHashMap<Long, HashMap<Integer, MVChunk>>();
    ...
    private MVWriteBuffer writeBuffer;
    ...
    MVStore(HashMap<String, Object> config) {
        ...
        meta = new MVMap<String, String>(MVStringDataType.INSTANCE,
                MVStringDataType.INSTANCE);
        HashMap<String, Object> c = New.hashMap();
        c.put("id", 0);
        c.put("createVersion", currentVersion);
        meta.init(this, c);
        fileStore = (MVFileStore) config.get("fileStore");
        ...
        if (mb > 0) {
            MVCacheLongKeyLIRS.Config cc = new MVCacheLongKeyLIRS.Config();
            cc.maxMemory = mb * 1024L * 1024L;
            cache = new MVCacheLongKeyLIRS<MVPage>(cc);
            cc.maxMemory /= 4;
            cacheChunkRef = new MVCacheLongKeyLIRS<PageChildren>(cc);
        }
    ...
    private MVMap<String, String> getMetaMap(long version) {
        MVChunk c = getChunkForVersion(version);
    ...
    private MVChunk getChunkForVersion(long version) {
        MVChunk newest = null;
        for (MVChunk c : chunks.values()) {
    ...
    private synchronized void readStoreHeader() {
        ...
        for (int i = 0; i <= BLOCK_SIZE; i += BLOCK_SIZE) {
            ...
            try {
                ...
                if (newest == null || version > newest.version) {
                    ...
                    MVChunk test = readChunkHeaderAndFooter(block);
        ...
        MVChunk test = readChunkFooter(fileStore.size());
        if (test != null) {
    ...
    private void loadChunkMeta() {
        ...
        for (Iterator<String> it = meta.keyIterator("chunk."); it.hasNext();) {
            ...
            s = meta.get(s);
            MVChunk c = MVChunk.fromString(s);
    ...
    private void setLastChunk(MVChunk last) {
        lastChunk = last;
        if (last == null) {
            // no valid chunk
            lastMapId = 0;
    ...
    private void verifyLastChunks() {
        ...
        for (Integer chunkId : ids) {
            ...
            MVChunk test = readChunkHeaderAndFooter(c.block);
            if (test == null || test.id != c.id) {
                break;
            }
            newestValidChunk = chunkId;
        }
        MVChunk newest = chunks.get(newestValidChunk);
        if (newest != lastChunk) {
     ...
     *         consistent
     */
    private MVChunk readChunkHeaderAndFooter(long block) {
        MVChunk header;
        try {
        ...
        MVChunk footer = readChunkFooter((block + header.len) * BLOCK_SIZE);
     ...
     */
    private MVChunk readChunkFooter(long end) {
        // the following can fail for various reasons
        try {
            // read the chunk footer of the last block of the file
            ByteBuffer lastBlock = fileStore.readFully(end - MVChunk.FOOTER_LENGTH, MVChunk.FOOTER_LENGTH);
            byte[] buff = new byte[MVChunk.FOOTER_LENGTH];
            lastBlock.get(buff);
    ...
    boolean isChunkLive(int chunkId) {
        String s = meta.get(MVChunk.getMetaKey(chunkId));
        return s != null;
    }
     ...
     * @return the chunk
     */
    private MVChunk getChunk(long pos) {
        MVChunk c = getChunkIfFound(pos);
        if (c == null) {
    ...
    private MVChunk getChunkIfFound(long pos) {
        int chunkId = DataUtils.getPageChunkId(pos);
        MVChunk c = chunks.get(chunkId);
        if (c == null) {
            checkOpen();
    ...
    private long storeNowTry() {
        ...
        if (lastChunk == null) {
            ...
            meta.put(MVChunk.getMetaKey(lastChunkId), lastChunk.asString());
            // never go backward in time
            time = Math.max(lastChunk.time, time);
        }
        int newChunkId = lastChunkId;
        while (true) {
            newChunkId = (newChunkId + 1) % MVChunk.MAX_ID;
            MVChunk old = chunks.get(newChunkId);
            if (old == null) {
                break;
            }
        ...
        MVChunk c = new MVChunk(newChunkId);
        ...
        meta.put(MVChunk.getMetaKey(c.id), c.asString());
        meta.remove(MVChunk.getMetaKey(c.id));
        ...
        MVWriteBuffer buff = getWriteBuffer();
        ...
        int length = MathUtils.roundUpInt(chunkLength +
                MVChunk.FOOTER_LENGTH, BLOCK_SIZE);
        buff.limit(length);
        ...
        buff.position(buff.limit() - MVChunk.FOOTER_LENGTH);
        ...
        if (!storeAtEndOfFile) {
            if (lastChunk == null) {
                ...
                if (lastChunk.version - headerVersion > 20) {
                    ...
                    while (true) {
                        MVChunk old = chunks.get(chunkId);
                        if (old == null) {
    ...
    private synchronized void freeUnusedChunks() {
        ...
        ArrayList<MVChunk> free = New.arrayList();
        long time = getTimeSinceCreation();
        for (MVChunk c : chunks.values()) {
    ...
    private Set<Integer> collectReferencedChunks() {
        ...
        for (MVCursor<String, String> c = meta.cursor("root."); c.hasNext();) {
            String key = c.next();
     ...
     * @return the buffer
     */
    private MVWriteBuffer getWriteBuffer() {
        MVWriteBuffer buff;
        if (writeBuffer != null) {
            buff = writeBuffer;
            buff.clear();
        } else {
            buff = new MVWriteBuffer();
        }
        return buff;
    }
     ...
     */
    private void releaseWriteBuffer(MVWriteBuffer buff) {
        if (buff.capacity() <= 4 * 1024 * 1024) {
            writeBuffer = buff;
        }
    }

    private boolean canOverwriteChunk(MVChunk c, long time) {
        if (retentionTime >= 0) {
            if (c.time + retentionTime > time) {
                return false;
            }
    ...
    private void applyFreedSpace(long storeVersion) {
        while (true) {
            ArrayList<MVChunk> modified = New.arrayList();
            Iterator<Entry<Long, HashMap<Integer, MVChunk>>> it;
            it = freedPageSpace.entrySet().iterator();
            while (it.hasNext()) {
                Entry<Long, HashMap<Integer, MVChunk>> e = it.next();
                long v = e.getKey();
                if (v > storeVersion) {
                    continue;
                }
                HashMap<Integer, MVChunk> freed = e.getValue();
                for (MVChunk f : freed.values()) {
                    MVChunk c = chunks.get(f.id);
                    if (c == null) {
                        // already removed
                        continue;
                    }
    ...
    private MVChunk readChunkHeader(long block) {
        long p = block * BLOCK_SIZE;
        ByteBuffer buff = fileStore.readFully(p, MVChunk.MAX_HEADER_LENGTH);
        return MVChunk.readChunkHeader(buff, p);
    }
    ...
    public synchronized boolean compactRewriteFully() {
        ...
        for (MVMap<?, ?> m : maps.values()) {
            ...
            MVCursor<Object, Object> cursor = map.cursor(null);
            MVPage lastPage = null;
            while (cursor.hasNext()) {
                cursor.next();
                MVPage p = cursor.getPage();
                if (p == lastPage) {
                    continue;
                }
                Object k = p.getKey(0);
    ...
    public synchronized boolean compactMoveChunks(int targetFillRate, long moveSize) {
        ...
        try {
            ...
            ArrayList<MVChunk> move = compactGetMoveBlocks(start, moveSize);
    ...
    private ArrayList<MVChunk> compactGetMoveBlocks(long startBlock, long moveSize) {
        ArrayList<MVChunk> move = New.arrayList();
        for (MVChunk c : chunks.values()) {
        ...
        // sort by block
        Collections.sort(move, new Comparator<MVChunk>() {
            @Override
            public int compare(MVChunk o1, MVChunk o2) {
    ...
    private void compactMoveChunks(ArrayList<MVChunk> move) {
        for (MVChunk c : move) {
            MVWriteBuffer buff = getWriteBuffer();
            long start = c.block * BLOCK_SIZE;
            int length = c.len * BLOCK_SIZE;
            ...
            MVChunk.readChunkHeader(readBuff, start);
            ...
            buff.position(length - MVChunk.FOOTER_LENGTH);
            buff.put(c.getFooterBytes());
            buff.position(0);
        ...
        for (MVChunk c : move) {
            ...
            buff.limit(length);
            ByteBuffer readBuff = fileStore.readFully(start, length);
            MVChunk.readChunkHeader(readBuff, 0);
            int chunkHeaderLen = readBuff.position();
            buff.position(chunkHeaderLen);
            buff.put(readBuff);
    ...
    private ArrayList<MVChunk> compactGetOldChunks(int targetFillRate, int write) {
        if (lastChunk == null) {
            // nothing to do
            return null;
        }
        ...
        ArrayList<MVChunk> old = New.arrayList();
        MVChunk last = chunks.get(lastChunk.id);
        for (MVChunk c : chunks.values()) {
        ...
        Collections.sort(old, new Comparator<MVChunk>() {
            @Override
            public int compare(MVChunk o1, MVChunk o2) {
        ...
        for (Iterator<MVChunk> it = old.iterator(); it.hasNext();) {
            MVChunk c = it.next();
            if (move == c) {
                remove = true;
            } else if (remove) {
    ...
    private void compactRewrite(ArrayList<MVChunk> old) {
     ...
     * @return the page
     */
    MVPage readPage(MVMap<?, ?> map, long pos) {
        if (pos == 0) {
            throw DataUtils.newIllegalStateException(
                    DataUtils.ERROR_FILE_CORRUPT, "Position 0");
        }
        MVPage p = cache == null ? null : cache.get(pos);
        if (p == null) {
            MVChunk c = getChunk(pos);
            long filePos = c.block * BLOCK_SIZE;
            filePos += DataUtils.getPageOffset(pos);
            ...
            long maxPos = (c.block + c.len) * BLOCK_SIZE;
            p = MVPage.read(fileStore, pos, map, filePos, maxPos);
    ...
    private void registerFreePage(long version, int chunkId,
        ...
        HashMap<Integer, MVChunk> freed = freedPageSpace.get(version);
        if (freed == null) {
            freed = New.hashMap();
            HashMap<Integer, MVChunk> f2 = freedPageSpace.putIfAbsent(version,
                    freed);
            if (f2 != null) {
                freed = f2;
            }
        ...
        synchronized (freed) {
            MVChunk f = freed.get(chunkId);
            if (f == null) {
                f = new MVChunk(chunkId);
                freed.put(chunkId, f);
            }
        ...
        try {
            for (Iterator<String> it = oldMeta.keyIterator("chunk.");
                ...
                if (!meta.containsKey(chunkKey)) {
                    ...
                    MVChunk c2 = MVChunk.fromString(s);
                    MVChunk test = readChunkHeaderAndFooter(c2.block);
     ...
     */
    public MVFileStore getFileStore() {
        return fileStore;
    }
     ...
     */
    void cachePage(long pos, MVPage page, int memory) {
        if (cache != null) {
     ...
     * @return the cache
     */
    public MVCacheLongKeyLIRS<MVPage> getCache() {
        return cache;
    }
    ...
    public static class Builder {
         ...
         * @return this
         */
        public Builder fileStore(MVFileStore store) {
