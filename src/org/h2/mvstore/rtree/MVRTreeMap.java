...
import org.h2.mvstore.MVCursorPos;
import org.h2.mvstore.DataUtils;
import org.h2.mvstore.MVMap;
import org.h2.mvstore.MVPage;
 ...
 */
public class MVRTreeMap<V> extends MVMap<MVSpatialKey, V> {

    /**
     * The spatial key type.
     */
    final MVSpatialDataType keyType;

    private boolean quadraticSplit;

    public MVRTreeMap(int dimensions, MVDataType valueType) {
        super(new MVSpatialDataType(dimensions), valueType);
        this.keyType = (MVSpatialDataType) getKeyType();
    }
     ...
     * @return the map
     */
    public static <V> MVRTreeMap<V> create(int dimensions, MVDataType valueType) {
     ...
     * @return the iterator
     */
    public RTreeCursor findIntersectingKeys(MVSpatialKey x) {
        return new RTreeCursor(root, x) {
            @Override
            protected boolean check(boolean leaf, MVSpatialKey key,
                    MVSpatialKey test) {
     ...
     * @return the iterator
     */
    public RTreeCursor findContainedKeys(MVSpatialKey x) {
        return new RTreeCursor(root, x) {
            @Override
            protected boolean check(boolean leaf, MVSpatialKey key,
                    MVSpatialKey test) {
    ...
    private boolean contains(MVPage p, int index, Object key) {
     ...
     */
    protected Object get(MVPage p, Object key) {
        if (!p.isLeaf()) {
    ...
    @Override
    protected synchronized Object remove(MVPage p, long writeVersion, Object key) {
        Object result = null;
        if (p.isLeaf()) {
            for (int i = 0; i < p.getKeyCount(); i++) {
                if (keyType.equals(p.getKey(i), key)) {
        ...
        for (int i = 0; i < p.getKeyCount(); i++) {
            if (contains(p, i, key)) {
                ...
                MVPage c = cOld.copy(writeVersion);
    ...
    private Object getBounds(MVPage x) {
    ...
    @Override
    @SuppressWarnings("unchecked")
    public V put(MVSpatialKey key, V value) {
     ...
     */
    public void add(MVSpatialKey key, V value) {
        putOrAdd(key, value, true);
    }

    private synchronized Object putOrAdd(MVSpatialKey key, V value, boolean alwaysAdd) {
        beforeWrite();
        long v = writeVersion;
        MVPage p = root.copy(v);
        Object result;
        if (alwaysAdd || get(key) == null) {
            if (p.getMemory() > store.getPageSplitSize() &&
                ...
                MVPage split = split(p, v);
                Object k1 = getBounds(p);
                Object k2 = getBounds(split);
                Object[] keys = { k1, k2 };
                MVPage.PageReference[] children = {
                        new MVPage.PageReference(p, p.getPos(), p.getTotalCount()),
                        new MVPage.PageReference(split, split.getPos(), split.getTotalCount()),
                        new MVPage.PageReference(null, 0, 0)
                };
                p = MVPage.create(this, v,
                        keys, null,
                        children,
                        totalCount, 0);
     ...
     */
    private Object set(MVPage p, long writeVersion, Object key, Object value) {
        if (p.isLeaf()) {
            for (int i = 0; i < p.getKeyCount(); i++) {
                if (keyType.equals(p.getKey(i), key)) {
    ...
    private void add(MVPage p, long writeVersion, Object key, Object value) {
        if (p.isLeaf()) {
        ...
        MVPage c = p.getChildPage(index).copy(writeVersion);
        if (c.getMemory() > store.getPageSplitSize() && c.getKeyCount() > 4) {
            // split on the way down
            MVPage split = split(c, writeVersion);
    ...
    private MVPage split(MVPage p, long writeVersion) {
        return quadraticSplit ?
                splitQuadratic(p, writeVersion) :
                splitLinear(p, writeVersion);
    }

    private MVPage splitLinear(MVPage p, long writeVersion) {
        ...
        MVPage splitA = newPage(p.isLeaf(), writeVersion);
        MVPage splitB = newPage(p.isLeaf(), writeVersion);
    ...
    private MVPage splitQuadratic(MVPage p, long writeVersion) {
        MVPage splitA = newPage(p.isLeaf(), writeVersion);
        MVPage splitB = newPage(p.isLeaf(), writeVersion);
    ...
    private MVPage newPage(boolean leaf, long writeVersion) {
        Object[] values;
        MVPage.PageReference[] refs;
        if (leaf) {
            values = MVPage.EMPTY_OBJECT_ARRAY;
            refs = null;
        } else {
            values = null;
            refs = new MVPage.PageReference[] {
                    new MVPage.PageReference(null, 0, 0)};
        }
        return MVPage.create(this, writeVersion,
                MVPage.EMPTY_OBJECT_ARRAY, values,
                refs, 0, 0);
    }

    private static void move(MVPage source, MVPage target, int sourceIndex) {
        Object k = source.getKey(sourceIndex);
        if (source.isLeaf()) {
            Object v = source.getValue(sourceIndex);
            target.insertLeaf(0, k, v);
        } else {
            MVPage c = source.getChildPage(sourceIndex);
     ...
     */
    public void addNodeKeys(ArrayList<MVSpatialKey> list, MVPage p) {
        if (p != null && !p.isLeaf()) {
            for (int i = 0; i < p.getKeyCount(); i++) {
                list.add((MVSpatialKey) p.getKey(i));
    ...
    @Override
    protected int getChildPageCount(MVPage p) {
     ...
     */
    public static class RTreeCursor implements Iterator<MVSpatialKey> {

        private final MVSpatialKey filter;
        private MVCursorPos pos;
        private MVSpatialKey current;
        private final MVPage root;
        private boolean initialized;

        protected RTreeCursor(MVPage root, MVSpatialKey filter) {
            this.root = root;
            this.filter = filter;
        }
        ...
        public boolean hasNext() {
            if (!initialized) {
                // init
                pos = new MVCursorPos(root, 0, null);
                fetchNext();
                initialized = true;
            }
            return current != null;
        ...
        @Override
        public MVSpatialKey next() {
            if (!hasNext()) {
                return null;
            }
            MVSpatialKey c = current;
            fetchNext();
            return c;
        }
            ...
            while (pos != null) {
                ...
                if (p.isLeaf()) {
                    while (pos.index < p.getKeyCount()) {
                        MVSpatialKey c = (MVSpatialKey) p.getKey(pos.index++);
                    ...
                    while (pos.index < p.getKeyCount()) {
                        int index = pos.index++;
                        MVSpatialKey c = (MVSpatialKey) p.getKey(index);
                        if (filter == null || check(false, c, filter)) {
                            MVPage child = pos.page.getChildPage(index);
                            pos = new MVCursorPos(child, 0, pos);
                            found = true;
                            break;
                        }
                    }
         ...
         */
        protected boolean check(boolean leaf, MVSpatialKey key, MVSpatialKey test) {
            return true;
        }
    ...
    public static class Builder<V> implements
            MVMap.MapBuilder<MVRTreeMap<V>, MVSpatialKey, V> {

        private int dimensions = 2;
        private MVDataType valueType;
         ...
         * @return this
         */
        public Builder<V> valueType(MVDataType valueType) {
