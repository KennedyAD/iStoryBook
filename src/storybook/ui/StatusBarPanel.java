...
public class StatusBarPanel extends AbstractPanel implements ActionListener {
	...
	public void initUi() {
		...
		lbParts = new JLabel(
				" " + I18N.getMsgColon("msg.common.current.part") + " " + mainFrame.getCurrentPart().toString());
		add(lbParts);
	...
	public void modelPropertyChange(PropertyChangeEvent evt) {
		...
		if (BookController.ChapterProps.INIT.check(propName) || BookController.ChapterProps.DELETE.check(propName)
				|| BookController.ChapterProps.DELETE_MULTI.check(propName)
				|| BookController.ChapterProps.NEW.check(propName) || BookController.ChapterProps.UPDATE.check(propName)
				|| BookController.SceneProps.INIT.check(propName) || BookController.SceneProps.DELETE.check(propName)
				|| BookController.SceneProps.DELETE_MULTI.check(propName)
				|| BookController.SceneProps.NEW.check(propName) || BookController.SceneProps.UPDATE.check(propName)) {
			refreshStat();
			// return;
		}
	...
	private void refreshLayoutCombo() {
		 ...
		 * Session session = model.beginTransaction(); PreferenceDAOImpl dao =
		 * new PreferenceDAOImpl(session); List<Preference> pref =
		 * dao.findAll(); for (Preference preference : pref) { if
		 * (preference.getKey().startsWith(PreferenceKey.DOCKING_LAYOUT.toString
		 * ())) { String name = preference.getStringValue(); if
		 * (SbConstants.BookKey.LAST_USED_LAYOUT.toString().equals(name)) {
		 * continue; } LoadDockingLayoutAction act = new
		 * LoadDockingLayoutAction(mainFrame, name); layoutCombo.addItem(act); }
		 * } model.commit();
		 */
	}
