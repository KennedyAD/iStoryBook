...
public class SbActionManager implements PropertyChangeListener {
	...
	/*
	 * private int findRecent(JMenu menu) { int rc = 2; for (int i = 0; i <
	 * menu.getItemCount(); i++) { if
	 * (menu.getItem(i).getLabel().equals(I18N.getMsg("msg.file.open.recent")))
	 * return (i); } return (rc); }
	 */
	...
	public void propertyChange(PropertyChangeEvent evt) {
		...
		if (BookController.PartProps.NEW.check(propName) || BookController.PartProps.UPDATE.check(propName)
	 ...
	 * "main-entities-menu-command", "secondary-entities-menu-command",
	 * "new-entity-menu-command", "parts-menu-command", "charts-menu-command",
	 * "tools-menu-command", "langtool-command", "window-menu-command",
	 * "preferences-command", "help-menu", "report-bug-command", "doc-command",
	 * "faq-command", "homepage-command", "facebook-command",
	 * "check-update-command", "about-command", "trace-command" }; List<String>
	 * list = Arrays.asList(actionNames); Set<String> ids =
	 * actionManager.getActionIDs(); for (String id : ids) { if
	...
	private void reloadPartMenu(JMenuBar menubar) {
		...
		for (Part part : parts) {
			Action action = new ChangePartAction(I18N.getMsg("msg.common.part") + " " + part.getNumberName(),
					actionHandler, part);
