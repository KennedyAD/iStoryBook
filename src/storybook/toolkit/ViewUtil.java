...
public class ViewUtil {
	...
	public static void scrollToTop(final JScrollPane scroller, int delay) {
		 ...
		 * 
		 * @Override public void actionPerformed(ActionEvent e) { // make sure
		 * the scroller is on top //
		 * scroller.getVerticalScrollBar().setValue(0); //
		 * scroller.getHorizontalScrollBar().setValue(0); JViewport viewport =
		 * scroller.getViewport(); JComponent comp = (JComponent)
		 * viewport.getView(); comp.scrollRectToVisible(new Rectangle(0, 0, 10,
		 * 10)); } }); timer.setRepeats(false); timer.start();
		 */
