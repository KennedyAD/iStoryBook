 ...
 * extension. Each image should have the same basename followed by its index,
 * ranging 1 to x. If a 0 index image exists at the location then it is used
 * (even though it is not counted in the nrOfPages for the setPages() method).
 *
 * Navigating through the book can be done by clicking in the bottomright or
 * bottomleft corner, by using the 4 arrow keys or by dragging and dropping a
 * page. To turn pages programmatically either call nextPage() or
 * previousPage(). Jumping to a page can be done by calling setLeftPageIndex().
 *
 * By default soft clipping is off and the borders at the edges of the papers
 * are visible.
 *
 * fade drop shadow on percentage turned (see passedThreshold for calculations)
 *
...
public class JBookPanel extends JPanel implements MouseInputListener, ActionListener {
	...
	public void paintComponent(Graphics g) {
		...
		// page 2
		paintPage(g2, currentRightImage, bookBounds.x + pageWidth, bookBounds.y, pageWidth, bookBounds.height, this,
				true);
	...
	protected void paintLeftPage(Graphics2D g2) {
		...
		// add drop shadow
		GradientPaint grad = new GradientPaint(bookBounds.x + done, bookBounds.height + bookBounds.y, shadowDarkColor,
	...
	protected void paintLeftPageSoftClipped(Graphics2D g2) {
		...
		gImg.fillRect(0, 0 - bookBounds.height - bookBounds.height, pageWidth + 10, bookBounds.height); // remove
																										// the
																										// top
																										// margin
																										// from
		...
		gImg.setColor(new Color(255, 255, 255, 0));
		gImg.fillRect(0, 0, this.getWidth(), bookBounds.y); // remove the top
															// margin from
															// allowed area
		gImg.setComposite(AlphaComposite.SrcAtop);
	...
	protected void paintRightPageSoftClipped(Graphics2D g2) {
		...
		gImg.fill(clipPath);
		gImg.setColor(new Color(255, 255, 255, 0));
		gImg.fillRect(-10, 0 - bookBounds.height - bookBounds.height, pageWidth + 10, bookBounds.height); // remove
																											// the
																											// top
																											// margin
																											// from
		...
		// paint page
		paintPage(gImg, nextRightImage, pageWidth + bookBounds.x, bookBounds.y, pageWidth, bookBounds.height, this,
				true);
